...
function Screen({ children, bg="#f7f9f6" }) {
  return (
    <div style={{
...
function PinPad({ value, onChange, onSubmit, error, label, sublabel }) {
  ...
  return (
    <div style={{display:"flex", flexDirection:"column", alignItems:"center", gap:0}}>
      ...
      <div style={{display:"flex", gap:14, marginBottom:32}}>
        {[0,1,2,3,4,5].map(i => (
          <div key={i} style={{
            ...
            background: i < value.length ? "#3A6EA5" : "rgba(58,110,165,0.15)",
      ...
      {error && (
        <div style={{
          fontSize:13, color:"#D4523A", fontWeight:600,
          marginBottom:16, textAlign:"center",
          background:"rgba(212,82,58,0.08)", padding:"8px 16px",
          borderRadius:10,
        }}>{error}</div>
      )}
      ...
      <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, width:240}}>
        {digits.map((d, i) => (
          <button key={i} onClick={() => {
            ...
            fontSize: d === "⌫" ? 20 : 22,
            fontWeight:700, color: d === "⌫" ? "#D4523A" : "#2a3d2a",
...
function PinSetup({ onDone }) {
  ...
  async function handleFirst(pin) {
    if (pin.length === 6) { setFirst(pin); setStep("confirm"); }
  }
  ...
  async function handleConfirm(pin) {
    ...
    if (pin !== first) {
      ...
      setSecond(""); setStep("create"); setFirst(""); return;
    }
    setLoading(true);
    try {
      const hash = await hashPin(pin);
      await supabase.from("loculus_pin").insert({ pin_hash: hash });
      sessionStorage.setItem("loculus_auth", "1");
      onDone();
    } catch(e) { setError("Erro ao salvar PIN. Tente novamente."); }
    finally { setLoading(false); }
  }

  return (
    <Screen bg="linear-gradient(180deg,#e8f4ef 0%,#f0ede8 60%,#e8e4f0 100%)">
      <div style={{flex:1,display:"flex",flexDirection:"column",
        alignItems:"center",justifyContent:"center",padding:"32px 24px"}}>
        <div style={{width:80,height:80,borderRadius:"50%",overflow:"hidden",
          marginBottom:24,boxShadow:"0 8px 24px rgba(90,120,90,0.15)"}}>
          <img src={IMG_URL} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
        </div>
        <div style={{fontSize:32,fontWeight:900,color:"#2a3d2a",
          fontFamily:"'Georgia',serif",letterSpacing:"-1px",marginBottom:32}}>
          Loculus
        </div>
        {loading ? (
          <div style={{color:"#7a9a7a",fontSize:14}}>Salvando PIN…</div>
        ) : step === "create" ? (
          <PinPad value={first} onChange={setFirst} onSubmit={handleFirst} error={error}
            label="Crie seu PIN" sublabel="6 dígitos para proteger suas gavetas"/>
        ) : (
          <PinPad value={second} onChange={setSecond} onSubmit={handleConfirm} error={error}
            label="Confirme o PIN" sublabel="Digite novamente para confirmar"/>
        )}
      </div>
    </Screen>
  );
...
function PinLogin({ onSuccess, onForgot }) {
  ...
  async function handleSubmit(p) {
    ...
    try {
      ...
      const { data } = await supabase.from("loculus_pin").select("pin_hash").single();
      if (data?.pin_hash === hash) {
        sessionStorage.setItem("loculus_auth", "1");
        onSuccess();
      } else {
        const next = attempts + 1; setAttempts(next);
        setError(next >= 5 ? "Muitas tentativas. Esqueceu o PIN?" : "PIN incorreto. Tente novamente.");
        setPin("");
      }
    } catch(e) { setError("Erro de conexão."); setPin(""); }
    finally { setLoading(false); }
  }
  ...
  return (
    <Screen bg="linear-gradient(180deg,#e8f4ef 0%,#f0ede8 60%,#e8e4f0 100%)">
      <div style={{flex:1,display:"flex",flexDirection:"column",
        ...
        {loading ? (
          <div style={{color:"#7a9a7a",fontSize:14}}>Verificando…</div>
        ) : (
          <PinPad value={pin} onChange={setPin} onSubmit={handleSubmit} error={error}
            label="Digite seu PIN" sublabel=""/>
        )}
        {attempts >= 3 && (
          <button onClick={onForgot} style={{
            marginTop:24,background:"none",border:"none",
            color:"#D4523A",fontSize:13,fontWeight:700,cursor:"pointer",
          }}>Esqueci meu PIN</button>
        )}
      </div>
    </Screen>
  );
...
// ── Reminder Chip ───────────────────────────────────────────────
function ReminderChip({ r, cat, onToggle, onDelete }) {
  return (
    ...
    <div style={{
      ...
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        ...
        <input type="time" value={nTime} onChange={e=>setNTime(e.target.value)}
          style={{width:90,padding:"8px 10px",borderRadius:12,border:"1.5px solid #d4e4d4",
            fontSize:13,background:"#f0f7f0",outline:"none",color:"#2a3d2a"}}/>
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
        {categories.map(cat=>(
          <button key={cat.id} onClick={()=>setNCat(cat.id)} style={{
            ...
            {nCat!==cat.id&&<span style={{width:7,height:7,borderRadius:"50%",
...
// ── Bottom Bar ──────────────────────────────────────────────────
function BottomBar({ screen, onAdd, onMain, onCal, onLock }) {
  return (
    <div style={{
      ...
      <button onClick={onLock} style={{...btnIcon(),width:44,height:44,fontSize:18}}>🔒</button>
      <button onClick={onMain} style={{
        ...btnIcon(screen==="main"?"#3A6EA5":"transparent",
                   screen==="main"?"white":"#6a8a72"),
      ...
      <button onClick={onCal} style={{
        ...btnIcon(screen==="calendar"?"#3A6EA5":"transparent",
                   screen==="calendar"?"white":"#6a8a72"),
        width:44,height:44,fontSize:18,
      }}>📅</button>
      <div style={{width:44}}/>
    </div>
...
export default function App() {
  ...
  function lock() {
    sessionStorage.removeItem("loculus_auth");
    setAuthState("login"); setScreen("home");
  }

  async function handleForgotPin() {
    if (!confirm("Isso vai apagar seu PIN. Você precisará criar um novo. Continuar?")) return;
  ...
  async function toggleDone(id, currentDone) {
    ...
    const { error } = await supabase.from("loculus_reminders")
      .update({ done: !currentDone }).eq("id", id);
  ...
  async function updateCategory(id) {
    const { error } = await supabase.from("loculus_categories")
      .update({ label: editLabel, color: editColor }).eq("id", id);
    if (error) { alert("Erro: " + error.message); return; }
  ...
  const filtered = activeCat==="todos"       ? reminders
    : activeCat==="urgente"                  ? reminders.filter(r=>r.urgente&&!r.done)
    : activeCat==="importante"               ? reminders.filter(r=>r.importante&&!r.done)
    : reminders.filter(r=>r.category_id===activeCat);

  const byDate = filtered.reduce((acc,r) => {
    acc[r.date]=acc[r.date]||[]; acc[r.date].push(r); return acc;
  }, {});
  const sortedDates = Object.keys(byDate).sort();
  const urgentCount = reminders.filter(r=>r.urgente&&!r.done).length;
  ...
  function remsOnDay(d) { return reminders.filter(r=>r.date===ds(d)); }
  ...
  function DayCell({ d, size="week" }) {
    ...
    const hasUrgent=remsOnDay(d).some(r=>r.urgente&&!r.done);
    ...
    return (
      <div onClick={()=>setSelectedDay(isSel?null:dStr)} style={{
        ...
        <div style={{fontSize:small?13:15,fontWeight:800,
          color:isSel?"white":isToday?"#3A6EA5":"#2a3d2a"}}>{d.getDate()}</div>
        {count>0&&(
          <div style={{display:"flex",gap:2,justifyContent:"center",marginTop:3}}>
            <div style={{width:small?5:6,height:small?5:6,borderRadius:"50%",
              background:isSel?"white":hasUrgent?"#D4523A":"#7DBE8E"}}/>
            {!small&&count>1&&<div style={{width:6,height:6,borderRadius:"50%",
              background:isSel?"rgba(255,255,255,0.6)":"#9B7EC8"}}/>}
          </div>
        )}
      </div>
    );
  ...
  if (authState === "loading") return (
    <Screen bg="linear-gradient(180deg,#e8f4ef 0%,#f0ede8 60%,#e8e4f0 100%)">
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",
        color:"#7a9a7a",fontSize:14}}>Abrindo gavetas… 🗂️</div>
    </Screen>
  );
  ...
  if (screen === "home") return (
    <Screen bg="linear-gradient(180deg,#e8f4ef 0%,#f0ede8 60%,#e8e4f0 100%)">
      <div style={{flex:1,display:"flex",flexDirection:"column",
        alignItems:"center",justifyContent:"space-between",padding:"24px 32px 48px"}}>
        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <div style={{width:280,height:280,borderRadius:"50%",overflow:"hidden",
            boxShadow:"0 16px 60px rgba(90,120,90,0.18),0 4px 16px rgba(0,0,0,0.08)",
            border:"3px solid rgba(255,255,255,0.8)"}}>
            <img src={IMG_URL} alt="Loculus"
              style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"center"}}/>
          </div>
        </div>
        <div style={{textAlign:"center",marginBottom:40}}>
          <div style={{fontSize:48,fontWeight:900,letterSpacing:"-2px",color:"#2a3d2a",
            fontFamily:"'Georgia','Times New Roman',serif",lineHeight:1.1,marginBottom:10}}>
            Loculus
          </div>
          <div style={{fontSize:15,color:"#6a8a72",fontWeight:500,
            letterSpacing:"0.04em",fontStyle:"italic"}}>
            as gavetas do seu cérebro
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 20px 100px",scrollbarWidth:"none"}}>
        {categories.map(cat=>(
          <div key={cat.id} style={{display:"flex",alignItems:"center",gap:10,
            padding:"12px 14px",borderRadius:16,background:"white",marginBottom:8,
            boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
            <div style={{width:26,height:26,borderRadius:"50%",background:cat.color,flexShrink:0}}/>
            {editCatId===cat.id ? (
              <>
                <input value={editLabel} onChange={e=>setEditLabel(e.target.value)}
                  style={{flex:1,border:"1px solid #ddd",borderRadius:8,padding:"4px 8px",fontSize:14}}/>
                ...
                <button onClick={()=>updateCategory(cat.id)}
                  style={{...btnIcon("#3A6EA5","white"),fontSize:12,width:32,height:32}}>✓</button>
              </>
            ) : (
              <>
                <span style={{flex:1,fontSize:15,fontWeight:600,color:"#2a3d2a"}}>{cat.label}</span>
                <button onClick={()=>{setEditCatId(cat.id);setEditLabel(cat.label);setEditColor(cat.color);}}
        ...
        <div style={{padding:"14px 16px",borderRadius:16,marginTop:8,
          background:"rgba(58,110,165,0.07)",border:"1.5px dashed rgba(58,110,165,0.3)"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#3A6EA5",marginBottom:10}}>Nova gaveta</div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <input placeholder="Nome" value={newCatLabel} onChange={e=>setNewCatLabel(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&addCategory()}
              style={{flex:1,border:"1px solid #ddd",borderRadius:10,padding:"8px 12px",fontSize:14,outline:"none"}}/>
            <input type="color" value={newCatColor} onChange={e=>setNewCatColor(e.target.value)}
              style={{width:38,height:38,border:"none",borderRadius:10,cursor:"pointer",padding:2}}/>
            <button onClick={addCategory}
              style={{...btnIcon("#3A6EA5","white"),width:38,height:38,fontSize:18}}>+</button>
          </div>
        </div>
      </div>
    </Screen>
    ...
    return (
      <Screen bg="#f7f9f6">
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
          ...
          <span style={{fontSize:13,fontWeight:700,color:"#4a6a4a",textTransform:"capitalize"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4,marginBottom:4}}>
            {WEEKDAYS.map(d=>(
              <div key={d} style={{textAlign:"center",fontSize:10,fontWeight:700,
                color:"#8aaa8a",paddingBottom:3}}>{d}</div>
            ))}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:calView==="month"?3:4}}>
            {(calView==="week"?weekDays:monthDays).map((d,i)=>(
              <DayCell key={i} d={d} size={calView}/>
            ))}
          </div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"14px 16px 100px",scrollbarWidth:"none"}}>
          {selectedDay ? (
            <>
              <div style={{fontSize:13,fontWeight:700,color:"#8aaa8a",
                textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>
                {formatDateLabel(selectedDay)}
              </div>
              {reminders.filter(r=>r.date===selectedDay).length===0
                ? <div style={{textAlign:"center",color:"#bbb",fontSize:14,marginTop:20}}>
                    Nenhum lembrete nesse dia 🌿
                  </div>
                : <div style={{display:"flex",flexDirection:"column",gap:7}}>
                    {reminders.filter(r=>r.date===selectedDay).map(r=>(
                      <ReminderChip key={r.id} r={r} cat={getCat(categories,r.category_id)}
                        onToggle={toggleDone} onDelete={deleteReminder}/>
                    ))}
                  </div>
              }
            </>
          ) : (
            <div style={{textAlign:"center",color:"#bbb",fontSize:13,marginTop:20}}>
              Toque em um dia para ver os lembretes
            </div>
          )}
        </div>
        {showAdd&&<AddSheet categories={categories} nText={nText} setNText={setNText}
          nCat={nCat} setNCat={setNCat} nDate={nDate} setNDate={setNDate}
          nTime={nTime} setNTime={setNTime} nUrgente={nUrgente} setNUrgente={setNUrgente}
          nImportante={nImportante} setNImportante={setNImportante}
          onAdd={addReminder} onClose={()=>setShowAdd(false)} inputRef={inputRef} loading={saving}/>}
        <BottomBar screen="calendar" onAdd={()=>setShowAdd(true)}
          onMain={()=>{setScreen("main");setCalOffset(0);}} onCal={()=>{}} onLock={lock}/>
      </Screen>
    );
  }
  ...
  return (
    <Screen bg="#f7f9f6">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
        ...
        <span style={{fontSize:20,fontWeight:900,color:"#2a3d2a",
          fontFamily:"'Georgia',serif",letterSpacing:"-0.5px"}}>Loculus</span>
        <div style={{position:"relative"}}>
          <button onClick={()=>setScreen("catEdit")} style={btnIcon()}>🗂️</button>
          {urgentCount>0&&(
            <div style={{position:"absolute",top:-2,right:-2,
              width:16,height:16,borderRadius:"50%",
              background:"#D4523A",color:"white",fontSize:10,fontWeight:700,
              display:"flex",alignItems:"center",justifyContent:"center",
              border:"2px solid #f7f9f6"}}>{urgentCount}</div>
          )}
        </div>
      </div>
        ...
        {categories.map(cat=>{
          ...
          return (
            <button key={cat.id} onClick={()=>setActiveCat(cat.id)} style={{
              ...
              {!isActive&&<span style={{width:7,height:7,borderRadius:"50%",
                background:cat.color,display:"inline-block"}}/>}
              {cat.label}
            </button>
          );
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 14px",scrollbarWidth:"none"}}>
        {loading ? (
          <div style={{textAlign:"center",color:"#bbb",fontSize:14,marginTop:60}}>
            Abrindo gavetas… 🗂️
          </div>
          ...
          <div key={date}>
            <div style={{textAlign:"center",fontSize:11,color:"#9aaa9a",
              fontWeight:700,letterSpacing:"0.07em",textTransform:"uppercase",
              margin:"10px 0 8px"}}>{formatDateLabel(date)}</div>
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              {byDate[date].map(r=>(
                <ReminderChip key={r.id} r={r} cat={getCat(categories,r.category_id)}
                  onToggle={toggleDone} onDelete={deleteReminder}/>
      ...
      {showAdd&&<AddSheet categories={categories} nText={nText} setNText={setNText}
        ...
        nImportante={nImportante} setNImportante={setNImportante}
        onAdd={addReminder} onClose={()=>setShowAdd(false)} inputRef={inputRef} loading={saving}/>}
      <BottomBar screen="main" onAdd={()=>setShowAdd(true)}
        onMain={()=>{}} onCal={()=>setScreen("calendar")} onLock={lock}/>
    </Screen>
  );
}
